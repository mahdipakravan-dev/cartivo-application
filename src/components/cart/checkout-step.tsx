...
export function CheckoutStep({ items, onBack, onSuccess }: CheckoutStepProps) {
  ...
  const [sellerPrices, setSellerPrices] = useState<Record<string, number>>(() =>
    Object.fromEntries(
      checkoutItems.map((item) => [
        cartLineKey(item.partId, item.sellerId),
        item.displayedUnitPriceRial,
      ]),
    ),
  );
  ...
  const allPricesLocked = checkoutItems.every((item) =>
    isUsablePriceLock(
      priceLocks[cartLineKey(item.partId, item.sellerId)],
      item,
    ),
  );
  ...
  const loadCheckout = useCallback(async () => {
    ...
    setError("");
    setLockNotice("");
    const [addressResult, paymentResult, offerResults, lockResults] =
      await Promise.all([
        getAddresses()
          .then((value) => ({ value }))
        ...
        getPaymentMethods()
          ...
          .catch((reason: unknown) => ({ reason })),
        Promise.allSettled(
          checkoutItems.map((item) => getPartSellerOffers(item.partId)),
        ),
        Promise.allSettled(
          checkoutItems.map((item) =>
            createPriceLock(item.partId, item.sellerId),
          ),
        ),
      ]);
    ...
    if ("value" in addressResult) {
      ...
      setAddressId((current) =>
        addressResult.value.some((address) => address.id === current)
          ? current
          : (addressResult.value.find((address) => address.id != null)?.id ??
            null),
      );
      ...
      errors.push(
        errorMessage(addressResult.reason, "دریافت آدرس‌ها با خطا مواجه شد."),
      );
    }
    ...
    if ("value" in paymentResult) {
      ...
      setPaymentMethodId((current) =>
        paymentResult.value.some((method) => method.id === current)
          ? current
          : (paymentResult.value.find((method) => method.id != null)?.id ??
            null),
      );
    } else {
      errors.push(
        errorMessage(
          ...
          "دریافت روش‌های پرداخت با خطا مواجه شد.",
        ),
      );
    }
    ...
    const nextInvalidLines = offerResults.reduce<Record<string, string>>(
      (result, offerResult, index) => {
    ...
    const { locks, failures, authenticationRequired } = collectPriceLocks(
      checkoutItems,
      lockResults,
    );
    setPriceLocks(locks);
    Object.entries(locks).forEach(([key, lock]) => {
      const item = checkoutItems.find(
        (candidate) =>
          cartLineKey(candidate.partId, candidate.sellerId) === key,
      );
    ...
    if (failures.length > 0)
      setLockNotice(
        "قفل قیمت برای برخی اقلام ایجاد نشد؛ هنگام ثبت سفارش قیمت جاری فروشنده استفاده می‌شود.",
      );
    if (Object.keys(nextInvalidLines).length > 0)
      errors.push(
        "برای ادامه باید فروشنده اقلام نامعتبر را دوباره انتخاب کنید.",
      );
  ...
  const finalize = async () => {
    ...
    try {
      const { locks, failures } = await validateOrRenewLocks(
        checkoutItems,
        priceLocks,
      );
      setPriceLocks(locks);
      if (failures.length > 0)
        setLockNotice(
          "برخی قفل‌های قیمت تمدید نشدند؛ سفارش با قیمت جاری فروشنده ثبت می‌شود.",
        );
      ...
      if (isApiError(requestError, 400, 404)) {
        ...
        if (requestError.status === 404) {
          const [nextAddresses, nextPayments] = await Promise.allSettled([
            getAddresses(),
            getPaymentMethods(),
          ]);
          if (nextAddresses.status === "fulfilled") {
            ...
            if (
              !nextAddresses.value.some((address) => address.id === addressId)
            )
              setAddressId(null);
          }
      ...
      if (!isApiError(requestError) && orderRequest) {
        try {
          ...
          const matchingOrder = findMatchingRecentOrder(
          ...
          setError(
            "پاسخ ثبت سفارش دریافت نشد، اما سفارش مشابهی در تاریخچه پیدا نشد. می‌توانید دوباره تلاش کنید.",
          );
        } catch {
          setError(
            "ارتباط هنگام ثبت سفارش قطع شد و بررسی تاریخچه سفارش‌ها نیز ممکن نبود. پیش از تلاش دوباره، سفارش‌های حساب خود را بررسی کنید.",
          );
        }
      } else {
        setError(
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-6">
        <section className="rounded-[1.75rem]  bg-white p-5 sm:p-7">
          <h2 className="font-black text-dark">اقلام و فروشندگان انتخاب‌شده</h2>
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="mt-1 text-[10px] text-text-secondary">
                      {isUsablePriceLock(lock, item)
                        ? `قفل‌شده تا ${formatDateTime(lock.expiresAt)}`
                        : "قیمت جاری فروشنده"}
                    </p>
                  </div>
                </div>
        ...
        <section className="rounded-[1.75rem]  bg-white p-5 sm:p-7">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="flex size-10 items-center justify-center rounded-xl bg-accent text-[var(--primary)]">
                <MapPin className="size-5" />
              </span>
              <div>
                <h2 className="font-black text-dark">آدرس تحویل</h2>
                <p className="mt-1 text-xs text-text-secondary">
                  سفارش به این آدرس ارسال می‌شود
                </p>
              </div>
            </div>
            {addresses.length > 0 && (
              <Button
                ...
                variant="ghost"
                size="sm"
                onClick={() => setCreatingAddress((value) => !value)}
              >
                <Plus /> آدرس جدید
              </Button>
            )}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        addressId === address.id
                          ? "border-[var(--primary)] bg-accent/10 ring-2 ring-accent"
                          : "border-border hover:border-border",
                      )}
                    >
                      {addressId === address.id && (
                        <span className="absolute left-3 top-3 flex size-5 items-center justify-center rounded-full bg-primary text-white">
                      ...
                      <p
                        dir="ltr"
                        className="mt-2 text-right text-xs text-text-secondary"
                      >
        ...
        <section className="rounded-[1.75rem]  bg-white p-5 sm:p-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-dark">روش پرداخت</h2>
              <p className="mt-1 text-xs text-text-secondary">
                یکی از روش‌های فعال را انتخاب کنید
              </p>
            </div>
          </div>
                  ...
                  method.id != null && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        {method.persianName && method.englishName && (
                          <p className="mt-1 text-[10px] text-text-secondary">
                            {method.englishName}
                          </p>
                        )}
      ...
      <aside className="rounded-[1.75rem]  bg-white p-6 shadow-lg shadow-border/40 lg:sticky lg:top-24">
        <button
          type="button"
          onClick={onBack}
          className="mb-5 flex items-center gap-1 text-xs font-bold text-text-secondary hover:text-[var(--primary)]"
        >
          <ArrowRight className="size-4" /> بازگشت به سبد خرید
        </button>
        <h2 className="font-black text-dark">خلاصه نهایی سفارش</h2>
        <dl className="mt-5 space-y-4 text-sm">
          <div className="flex justify-between">
            <dt className="text-text-secondary">تعداد کالا</dt>
            <dd>
              {checkoutItems
          ...
          <div className="flex justify-between">
            <dt className="text-text-secondary">قیمت فروشندگان</dt>
            <dd>{lockedTotal.toLocaleString("fa-IR")} ریال</dd>
          </div>
        </dl>
        <div className="mt-5 border-t border-border pt-5">
          ...
          <p className="mt-2 text-[11px] leading-5 text-text-secondary">
            اعتبار کد هنگام ثبت نهایی سفارش توسط سرور بررسی می‌شود.
          </p>
        </div>
        <div className="my-5 h-px bg-border/20" />
        <div className="flex items-end justify-between">
          <span className="text-sm font-bold text-text-muted">
            {allPricesLocked ? "مبلغ قفل‌شده سفارش" : "مبلغ برآوردی سفارش"}
          </span>
          <b className="text-xl text-[var(--primary)]">
            {lockedTotal.toLocaleString("fa-IR")}{" "}
        ...
        {lockNotice && (
          <p
            role="status"
            className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800"
          >
            {lockNotice}
          </p>
        )}
        {error && (
          <p
            role="alert"
            className="mt-4 rounded-xl bg-red-50 p-3 text-xs leading-5 text-red-700"
          >
            {error}
          </p>
        )}
        ...
        <p className="mt-4 flex items-center justify-center gap-1.5 text-[11px] text-text-secondary">
          <ShieldCheck className="size-4 text-emerald-600" /> اطلاعات سفارش پیش
          از ثبت بررسی می‌شود
        </p>
      </aside>
    </div>
...
function isUsablePriceLock(
  lock: PriceLock | undefined,
  item: CartItem,
): lock is PriceLock & { lockToken: string; lockedPrice: number } {
  if (!lock?.lockToken || lock.lockedPrice == null || lock.valid === false)
    return false;
  if (lock.partId !== item.partId || lock.sellerId !== item.sellerId)
    return false;
  if (lock.expiresAt) {
...
function collectPriceLocks(
  items: CartItem[],
  results: PromiseSettledResult<PriceLock>[],
) {
  ...
  results.forEach((result, index) => {
    ...
    if (
      ...
      isUsablePriceLock(result.value, item)
    ) {
    ...
    const reason =
      result.status === "rejected"
        ? result.reason
        : new Error(`قفل قیمت «${item.name}» معتبر نیست.`);
...
async function validateOrRenewLocks(
  items: CartItem[],
  currentLocks: Record<string, PriceLock>,
) {
  ...
  const authenticationError = results.find(
    (result) =>
      result.status === "rejected" && isApiError(result.reason, 401, 403),
  );
  if (authenticationError?.status === "rejected")
    throw authenticationError.reason;
  return collected;
}

async function refreshInvalidLines(
  items: CartItem[],
  setInvalidLines: (lines: Record<string, string>) => void,
) {
  const results = await Promise.allSettled(
    items.map((item) => getPartSellerOffers(item.partId)),
  );
  const invalid = results.reduce<Record<string, string>>(
    (result, offerResult, index) => {
      const item = items[index];
      if (!item) return result;
      const key = cartLineKey(item.partId, item.sellerId);
      if (offerResult.status === "rejected")
        result[key] = errorMessage(
          offerResult.reason,
          "پیشنهاد فروشنده قابل بررسی نیست.",
        );
      else {
        const offer = offerResult.value.find(
          (candidate) =>
            candidate.sellerId === item.sellerId && candidate.priceRial != null,
        );
  ...
  return (
    <label className="block text-xs font-bold text-text-muted">
      ...
      <Input
        ...
        dir={phone ? "ltr" : "rtl"}
        aria-invalid={!!error}
        className="mt-1.5 h-11 rounded-xl font-normal"
        {...form.register(name, {
          required: required ? `${label} را وارد کنید.` : false,
          ...(phone
            ? {
                validate: (value) =>
                  /^(?:09\d{9}|\+989\d{9})$/.test(
                    toEnglishDigits(value || "").replace(/\s/g, ""),
