      ...
      <div className="container-cartivo px-4 sm:px-6 lg:px-8">
        <div className="relative isolate min-h-[330px] overflow-hidden rounded-[2rem] bg-primary text-primary-foreground shadow-[0_24px_70px_rgb(15_23_42/0.12)]">
          ...
          <div className="grid min-h-[330px] items-center lg:grid-cols-[1.1fr_.9fr]">
            <div className="relative z-10 px-6 py-12 sm:px-10 lg:px-14"><span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-bold text-cyan-100"><SearchX className="size-4" /> سرویس درخواست قطعه</span><h2 className="mt-5 text-3xl font-black leading-[1.45] text-primary-foreground sm:text-4xl">قطعه مورد نظر رو پیدا نکردی؟<span className="block text-cyan-300">درخواست قطعه ثبت کن</span></h2><p className="mt-4 max-w-xl text-sm leading-7 text-white/60">مشخصات قطعه را برای ما بنویس؛ کارشناسان کارتیوو نتیجه بررسی را از طریق پیامک و پروفایل به شما اطلاع می‌دهند.</p><Button onClick={start} className="mt-7 h-12 rounded-xl bg-primary-foreground px-5 text-sm font-extrabold text-primary hover:bg-primary-foreground/90"><ClipboardPlus className="size-4" /> ثبت درخواست قطعه <ArrowLeft className="size-4" /></Button></div>
            <div className="relative hidden h-full items-center justify-center lg:flex"><div className="absolute size-64 rounded-full " /><div className="absolute size-48 rounded-full border border-dashed border-cyan-200/15" /><ClipboardPlus className="size-28 text-cyan-200/20" strokeWidth={1} /><div className="absolute bottom-9 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-bold text-white/65">پیگیری آسان از پروفایل کاربری</div></div>
          </div>
        </div>
      </div>
