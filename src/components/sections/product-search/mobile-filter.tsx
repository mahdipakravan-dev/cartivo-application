  ...
  return (
    <>
      ...
      <button
        ...
        className="flex items-center gap-2 rounded-xl  bg-white px-4 py-2.5 text-sm font-medium text-text-muted transition-all hover:border-border hover:shadow-sm lg:hidden"
      >
      ...
      {open && (
        <>
          ...
          />
          <div className="fixed inset-y-0 right-0 z-50 flex w-80 max-w-[85vw] flex-col overflow-y-auto bg-[var(--background)] p-5 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <span className="text-sm font-bold text-dark">فیلترها</span>
              <button
                onClick={() => setOpen(false)}
                className="rounded-lg p-1.5 text-text-secondary transition-colors hover:bg-border/20 hover:text-text-muted"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
