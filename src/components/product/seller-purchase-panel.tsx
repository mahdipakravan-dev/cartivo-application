...
export function SellerPurchasePanel({
  ...
  const [effectivePrice, setEffectivePrice] = useState<EffectivePrice | null>(
    null,
  );
  ...
  const loadOffers = useCallback(async () => {
    ...
    try {
      ...
      setSelectedSellerId((current) => {
        if (
          current != null &&
          available.some((offer) => offer.sellerId === current)
        )
          return current;
        return null;
      });
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "دریافت فروشندگان این قطعه انجام نشد.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const selectedPrice =
    effectivePrice?.sellerId === selectedSellerId &&
    effectivePrice.finalCalculatedPrice != null
      ? effectivePrice.finalCalculatedPrice
      : selectedOffer?.priceRial;
  const selectedCartQuantity = selectedOffer
    ? (cartItems.find(
        (item) =>
          item.partId === partId && item.sellerId === selectedOffer.sellerId,
      )?.quantity ?? 0)
    : 0;
  ...
  const selectOffer = async (offer: CompleteOffer) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "قیمت این فروشنده قابل تأیید نیست. فروشنده دیگری را انتخاب کنید.",
        );
      }
  ...
  return (
    <div className={cn("space-y-5", className)}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="flex items-center gap-2 text-sm font-black text-dark">
            <Store className="size-4 text-accent" /> انتخاب فروشنده
          </p>
          <p className="mt-1 text-xs leading-5 text-text-secondary">
            برای خرید، یکی از پیشنهادهای معتبر را انتخاب کنید.
          </p>
        </div>
        <button
          type="button"
          ...
          disabled={loading}
          className="flex size-9 shrink-0 items-center justify-center rounded-xl  text-text-secondary transition hover:text-[var(--primary)] disabled:opacity-50"
      ...
      {error && (
        <p
          role="alert"
          className="rounded-xl border border-red-100 bg-red-50 p-3 text-xs leading-6 text-red-700"
        >
          {error}
        </p>
      )}
      ...
      {loading ? (
        <div className="flex min-h-28 items-center justify-center rounded-2xl bg-background">
        ...
        <div className="rounded-2xl border border-dashed border-amber-200 bg-amber-50/60 p-5 text-center">
          <p className="text-sm font-bold text-amber-900">
            این قطعه در حال حاضر فروشنده فعال ندارد.
          </p>
          <p className="mt-1 text-xs text-amber-700/70">
            پس از ثبت قیمت جدید، امکان خرید فعال می‌شود.
          </p>
        </div>
      ) : (
        <div
            ...
            return (
              ...
              >
                <span className="min-w-0">
                  <span className="flex items-center gap-2 text-sm font-black text-dark">
                    {offer.sellerName}
                    {index === 0 && (
                  ...
                  <span className="mt-1 block text-[10px] text-text-secondary">
                    به‌روزرسانی: {formatDate(offer.lastUpdatedAt)}
                  </span>
                </span>
                <span className="shrink-0 text-left">
                  <span className="block text-base font-black text-[var(--primary)]">
                    {offer.priceRial.toLocaleString("fa-IR")}
                  </span>
                  <span className="text-[10px] text-text-secondary">ریال</span>
                </span>
                {selected && (
                  <span className="absolute -left-2 -top-2 flex size-6 items-center justify-center rounded-full bg-primary text-white">
        ...
        <div className="rounded-2xl bg-background p-4">
          <div className="flex items-end justify-between gap-3">
            <span className="text-xs text-text-secondary">
              قیمت انتخاب‌شده از {selectedOffer.sellerName}
            </span>
            <strong className="text-xl text-[var(--primary)]">
              {selectedPrice.toLocaleString("fa-IR")}{" "}
          ...
          {effectivePrice?.validUntil && (
            <p className="mt-2 text-[10px] text-text-secondary">
              معتبر تا {formatDateTime(effectivePrice.validUntil)}
            </p>
          )}
        </div>
      )}
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <Button
          ...
          className="h-12 rounded-xl border-[var(--primary)] text-[var(--primary)]"
        >
          <Zap /> خرید فوری
        </Button>
      </div>
