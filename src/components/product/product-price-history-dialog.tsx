...
export function ProductPriceHistoryDialog({
  ...
  const [history, setHistory] = useState<HistoryState>({ status: "idle", points: [] });
  ...
  useEffect(() => {
    ...
    getPartPriceHistory(partId)
      .then((page) => {
        if (!active) return;
        setHistory({ status: "success", points: [...(page.content ?? [])].reverse() });
      })
  ...
  return (
    <div
      className="fixed inset-0 z-[140] flex items-end justify-center bg-slate-950/60 backdrop-blur-sm sm:items-center sm:p-4"
      onMouseDown={(event) => {
    ...
    >
      <section
        ...
        dir="rtl"
      >
        <div className="h-1.5 bg-gradient-to-l from-[#14305A] via-cyan-600 to-cyan-300" />
        <button
          type="button"
          onClick={() => onOpenChange(false)}
          className="absolute left-5 top-5 flex size-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
        ...
        <div className="px-6 pb-7 pt-8 sm:px-8 sm:pb-8">
          ...
          <h2 id="price-history-dialog-title" className="mt-4 text-xl font-black text-slate-900 sm:text-2xl">
            میانگین قیمت روزانه
          </h2>
          ...
          <div className="mt-6">
            {history.status === "idle" || history.status === "loading" ? (
              <div className="flex min-h-56 items-center justify-center rounded-2xl bg-slate-50">
                <LoaderCircle className="size-7 animate-spin text-[#14305A]" />
              </div>
            ) : history.status === "error" ? (
              <div role="alert" className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-red-100 bg-red-50 px-6 text-center">
                <TriangleAlert className="size-9 text-red-500" />
                <p className="mt-4 text-sm font-bold text-red-800">دریافت تاریخچه قیمت ممکن نشد</p>
                <p className="mt-2 text-xs leading-6 text-red-700/70">{history.message}</p>
              </div>
              ...
              <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 text-center">
                ...
                <p className="mt-4 text-sm font-bold text-slate-700">هنوز تاریخچه قیمتی ثبت نشده است</p>
                <p className="mt-2 text-xs text-slate-400">پس از ثبت قیمت روزانه، نمودار در این بخش نمایش داده می‌شود.</p>
              </div>
            ) : (
...
function PriceHistoryChart({ points }: { points: PriceHistoryPoint[] }) {
  ...
  if (chartPoints.length === 0) {
    return (
      <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 text-center">
        <History className="size-10 text-slate-300" />
        <p className="mt-4 text-sm font-bold text-slate-700">داده معتبری برای نمودار وجود ندارد</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-black text-slate-700">روند قیمت</p>
      ...
      >
        <path d={`M ${padding} ${height - padding} H ${width - padding}`} stroke="#cbd5e1" strokeWidth="1" fill="none" />
        ...
        {coordinates.map(({ x, y, point }) => (
          <circle key={`${point.priceDate}-${point.recordedAt ?? ""}`} cx={x} cy={y} r="5" fill="#14305A">
