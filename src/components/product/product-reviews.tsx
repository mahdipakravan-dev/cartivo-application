...
export function ProductReviews({ cars }: { cars: CarResponse[] }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "ثبت دیدگاه انجام نشد.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <section className="mt-8 rounded-[1.75rem]  bg-white p-5 shadow-[0_16px_50px_rgb(15_23_42/0.04)] sm:p-8">
      <div className="flex flex-col justify-between gap-5 border-b border-border pb-6 sm:flex-row sm:items-end">
        <div>
          <p className="text-xs font-bold text-accent">نظر خریداران</p>
          <h2 className="mt-2 text-2xl font-black text-dark">
            امتیاز و دیدگاه کاربران
          </h2>
          <p className="mt-2 text-sm text-text-secondary">
            دیدگاه‌ها برای خودروی سازگار انتخاب‌شده نمایش داده می‌شوند.
          </p>
        </div>
        <div className="flex items-center gap-3 rounded-2xl bg-background px-4 py-3">
          <div className="text-left">
            ...
            <p className="text-[10px] text-text-secondary">از ۵ امتیاز</p>
          </div>
      ...
      {validCars.length > 1 && (
        <label className="mt-6 block max-w-sm text-xs font-bold text-text-muted">
          خودروی موردنظر
          <select
            value={carId ?? ""}
            onChange={(event) => setCarId(Number(event.target.value))}
            className="mt-2 h-11 w-full rounded-xl  bg-white px-3 text-sm outline-none focus:border-primary"
          >
            {validCars.map((car) => (
      ...
      <div className="mt-7 grid gap-7 lg:grid-cols-[1fr_360px]">
        <div>
          {loading ? (
            ...
            <div className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-border text-center">
              <MessageSquareText className="size-10 text-text-secondary" />
        ...
        <form onSubmit={submit} className="h-fit rounded-2xl bg-background p-5">
          <h3 className="text-base font-black text-dark">دیدگاه شما</h3>
          <p className="mt-1 text-xs leading-5 text-text-secondary">
            دیدگاه پس از بررسی و تأیید منتشر می‌شود.
          </p>
          ...
          <textarea
            ...
            maxLength={1000}
            rows={5}
            placeholder="تجربه شما از این قطعه برای خودروی انتخاب‌شده..."
            className="mt-4 w-full resize-none rounded-xl  bg-white p-3 text-sm leading-6 outline-none placeholder:text-text-secondary focus:border-primary"
          />
          {error && (
          ...
          >
            {submitting ? (
              ...
              <>
                <Send className="size-4" /> ثبت دیدگاه
              </>
            )}
          </Button>
        </form>
    ...
    <article className="rounded-2xl  p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs font-bold text-text-muted">کاربر کارتیوُ</p>
...
function carName(car: CarResponse) {
  return (
    [car.brand?.persianName || car.brand?.englishName, car.model, car.trimLevel]
      .filter(Boolean)
      .join(" ") || "خودرو"
  );
}
