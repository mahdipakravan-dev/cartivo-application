...
const cleanPhone = (value: string) =>
  toEnglishDigits(value).replace(/\D/g, "").slice(0, 11);
const toInternationalPhone = (value: string) =>
  `+98${cleanPhone(value).slice(1)}`;
...
export function AccountDialog({
  ...
  const submitOtp = async ({ code }: OtpForm) => {
    ...
    try {
      ...
      setProfile(null);
      onOpenChange(false);
      const postLoginAction = window.sessionStorage.getItem(
      ...
      } else if (postLoginAction === "basket-checkout") {
        window.sessionStorage.removeItem("cartivo_post_login_action");
      } else {
        router.push("/profile");
      }
      ...
      setRequestError(
        error instanceof Error ? error.message : "کد واردشده معتبر نیست.",
      );
    }
  };
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-end justify-center bg-dark/55 p-0 backdrop-blur-sm sm:items-center sm:p-4"
      onMouseDown={(event) =>
    ...
    >
      <section
        ...
        className="relative w-full overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:max-w-md sm:rounded-[2rem]"
        dir="rtl"
      >
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          className="absolute left-4 top-5 rounded-full text-text-secondary"
          aria-label="بستن"
        ...
        <div className="px-6 pb-7 pt-7 sm:px-8">
          {step !== "profile" && (
            <div className="mb-7">
              <div className="mb-4 flex size-14 items-center justify-center rounded-2xl bg-primary text-white shadow-lg shadow-primary/20">
              ...
              >
                ورود به حساب کارتیوُ
              </h2>
              <p className="mt-2 text-sm leading-6 text-text-secondary">
                سریع و امن، فقط با شماره موبایل شما
              </p>
            </div>
          )}
          ...
          {step === "phone" && (
            <form
              onSubmit={phoneForm.handleSubmit(({ phoneNumber }) =>
                sendOtp(phoneNumber),
              )}
              noValidate
            >
              <label
                htmlFor="phoneNumber"
                className="mb-2 block text-sm font-bold text-text-muted"
              >
                شماره موبایل
              </label>
              <div className="relative" dir="ltr">
                <Input
                  ...
                  {...phoneForm.register("phoneNumber", {
                    ...
                    validate: (value) =>
                      /^09\d{9}$/.test(cleanPhone(value)) ||
                      "شماره موبایل باید مانند 09369514975 باشد.",
                    onChange: (event) =>
                      phoneForm.setValue(
                        "phoneNumber",
              ...
              <p className="mt-4 text-center text-xs leading-5 text-text-secondary">
                با ورود، قوانین و شرایط استفاده از کارتیوُ را می‌پذیرید.
              </p>
            </form>
          )}
          ...
          {step === "otp" && (
            <form onSubmit={otpForm.handleSubmit(submitOtp)} noValidate>
              <button
                ...
                className="mb-5 inline-flex items-center gap-1 text-sm font-bold text-[var(--primary)]"
              >
                <ArrowRight className="size-4" /> تغییر شماره
              </button>
              <p className="mb-5 text-sm leading-6 text-text-muted">
                کد ارسال‌شده به{" "}
                <b dir="ltr" className="inline-block text-dark">
                  {phone}
                </b>{" "}
                را وارد کنید.
              </p>
              ...
              <Input
                ...
                {...otpForm.register("code", {
                  ...
                  pattern: {
                    value: /^\d{5}$/,
                    message: "کد ورود باید ۵ رقم باشد.",
                  },
                  onChange: (event) =>
                    otpForm.setValue(
                      "code",
              ...
              />
              <FormMessage
                message={otpForm.formState.errors.code?.message || requestError}
              />
              <Button
                type="submit"
                size="lg"
              ...
              <div className="mt-5 text-center text-sm text-text-secondary">
                {remaining > 0 ? (
                  <>
                    ارسال دوباره تا{" "}
                    <span dir="ltr" className="font-bold text-text-muted">
          ...
          {step === "profile" && (
            <div>
              <div className="mb-6 flex items-center gap-3 border-b border-border pb-5">
                <div className="flex size-12 items-center justify-center rounded-2xl bg-accent text-[var(--primary)]">
                ...
                <div>
                  ...
                  <p
                    dir="ltr"
                    className="mt-1 text-right text-xs text-text-secondary"
                  >
              ...
              {loadingProfile ? (
                ...
                >
                    ...
                    <ProfileField
                      ...
                      rules={{
                        validate: (value: string) =>
                          !value ||
                          /^\d{10}$/.test(toEnglishDigits(value)) ||
                  ...
                  {saved && (
                    <p className="col-span-2 flex items-center gap-1 text-xs font-bold text-emerald-600">
                      <CheckCircle2 className="size-4" /> تغییرات ذخیره شد.
                    </p>
                  )}
                  <Button
                    type="submit"
                    className="col-span-2 mt-2 h-11 rounded-xl"
                  ...
                  <Button
                    type="button"
                    variant="ghost"
                    className="col-span-2 text-text-secondary hover:text-red-600"
                    onClick={logout}
                  >
                    <LogOut /> خروج از حساب
                  </Button>
...
function FormMessage({
  ...
  return message ? (
    <p
      role="alert"
      className={`mt-2 text-xs font-medium text-red-600 ${className}`}
    >
      {message}
    </p>
  ) : null;
  ...
  return (
    <label className="block text-xs font-bold text-text-muted">
      ...
      <Input
        ...
        inputMode={inputMode}
        aria-invalid={!!error}
        className="mt-1.5 h-10 rounded-xl font-normal"
