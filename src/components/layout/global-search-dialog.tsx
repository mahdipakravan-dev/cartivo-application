...
function ResultGroup({ title, icon: Icon, children }: { title: string; icon: typeof Search; children: React.ReactNode }) {
  return <section><div className="mb-3 flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-lg bg-primary text-cyan-200"><Icon className="size-4" /></span><h2 className="text-sm font-black text-slate-800">{title}</h2></div>{children}</section>;
}
