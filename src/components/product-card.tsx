    ...
    >
      <div
        className={cn(
          ...
          variant === "horizontal"
            ? "min-h-48 border-b border-border sm:min-h-full sm:border-b-0 sm:border-l"
            : "min-h-[210px] border-b border-border",
        )}
      >
        {primaryImage ? (
          <Image
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex size-20 items-center justify-center rounded-2xl  bg-white text-text-secondary shadow-sm">
        ...
        <div className="pointer-events-none absolute inset-x-5 bottom-2 h-5 rounded-[50%] bg-dark/5 blur-md" />
      </div>
      ...
      <div className="relative flex min-w-0 flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            {brandName && (
              <p className="mb-1.5 truncate text-[10px] font-bold text-accent">
                {brandName}
              </p>
            )}
            <h3 className="line-clamp-2 text-sm font-extrabold leading-6 text-dark transition-colors group-hover:text-primary sm:text-base sm:leading-7">
              {part.name || "قطعه خودرو"}
            </h3>
          </div>
          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-background text-text-secondary transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
            <ArrowLeft className="size-3.5 transition-transform group-hover:-translate-x-0.5" />
          </span>
        </div>

        {compatibleCarName && (
          <div className="mt-3 flex min-w-0 items-center gap-1.5 text-[10px] text-text-secondary">
            <CarFront className="size-3.5 shrink-0 text-text-secondary" />
            <span className="truncate">مناسب برای {compatibleCarName}</span>
            {(part.fitments?.length ?? 0) > 1 && (
              <span className="shrink-0">
                +{((part.fitments?.length ?? 1) - 1).toLocaleString("fa-IR")}
              </span>
            )}
          </div>
        )}
