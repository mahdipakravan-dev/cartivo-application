...
export async function generateMetadata({
  ...
  const { manufacturerName: encodedManufacturerName, categoryId } =
    await params;
  const manufacturerName = decodeRouteSegment(encodedManufacturerName);
  const numericCategoryId = parseId(categoryId);
  const category =
    numericCategoryId == null ? null : await getCategoryById(numericCategoryId);
  ...
  return {
    ...
    description: `مشاهده و خرید قطعات دسته‌بندی ${categoryName} از ${manufacturerName} در کارتیوُ`,
    alternates: {
      canonical: ROUTES.parentCategory(manufacturerName, numericCategoryId!),
    },
  };
}
...
export default async function ManufacturerCategoryPage({
  ...
  const [
    { manufacturerName: encodedManufacturerName, categoryId },
    rawSearchParams,
  ...
  const categoryName =
    category.persianName?.trim() || category.name?.trim() || "دسته‌بندی قطعات";
    ...
    <main className="bg-[var(--background)] pb-20 pt-24 sm:pt-28">
      <div className="container-cartivo px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative z-10 flex items-center px-6 py-12 sm:px-10 lg:px-14">
              <div>
                ...
                {category.name && category.name !== categoryName && (
                  <p dir="ltr" className="mt-2 w-fit text-sm text-white/45">
                    {category.name}
                  </p>
                )}
                <p className="mt-5 text-sm leading-8 text-white/60">
                  {results.totalElements.toLocaleString("fa-IR")} قطعه از{" "}
                  {manufacturerName} در این دسته‌بندی پیدا شد.
                </p>
              </div>
            </div>
        ...
        <section
          ...
          aria-labelledby="category-parts-title"
        >
          <div className="mb-7">
            <p className="text-xs font-bold text-accent">محصولات دسته‌بندی</p>
            <h2
              id="category-parts-title"
