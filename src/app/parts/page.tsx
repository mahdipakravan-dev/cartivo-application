      ...
      <div className="container-cartivo px-4 sm:px-6 lg:px-8">
        ...
        >
          <ol className="flex items-center gap-1.5">
            ...
            <li className="font-bold text-text-muted" aria-current="page">
              قطعات یدکی
            </li>
          </ol>
        </nav>
        ...
        <section className="relative isolate overflow-hidden rounded-[2rem] bg-primary shadow-[0_24px_70px_rgb(15_23_42/0.12)]">
          ...
          <div className="grid min-h-[350px] lg:grid-cols-[1.1fr_.9fr]">
            <div className="relative z-10 flex items-center px-6 py-11 sm:px-10 lg:px-14 lg:py-14">
              <div className="max-w-2xl">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-bold text-accent">
                  <PackageSearch className="size-4" /> بازار قطعات یدکی کارتیوُ
                </div>
                <h1 className="mt-5 text-3xl font-black leading-[1.35] text-white sm:text-4xl lg:text-5xl">
                  قطعه‌ی مناسب خودرو را
                  <span className="block text-accent">
                    سریع و دقیق پیدا کنید
                  </span>
                </h1>
                <p className="mt-4 max-w-xl text-sm leading-7 text-white/60 sm:text-base">
                  میان قطعات موجود جست‌وجو کنید و نتایج را بر اساس موقعیت، قیمت
                  و سازگاری محدود کنید.
                </p>
                <a
                  href="#parts-directory"
                  className="mt-7 inline-flex h-12 items-center gap-2 rounded-xl bg-white px-5 text-sm font-extrabold text-[var(--primary)] transition hover:-translate-y-0.5 hover:bg-accent"
                >
                  <Search className="size-4" /> جست‌وجوی قطعات{" "}
            ...
            <div className="relative hidden items-center justify-center border-r border-white/10 lg:flex">
              ...
              <div className="absolute size-56 rounded-full border border-dashed border-accent/20" />
              <PackageSearch
                className="size-32 text-accent/20"
                strokeWidth={1}
              />
              <Link
                href={ROUTES.brands}
                className="absolute right-10 top-10 flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-bold text-white/75 backdrop-blur-md hover:bg-white/15"
              >
                <CarFront className="size-4 text-accent" /> برند خودرو
              </Link>
              <Link
                href={ROUTES.partBrands}
                className="absolute bottom-10 left-10 flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-xs font-bold text-white/75 backdrop-blur-md hover:bg-white/15"
              >
                <Factory className="size-4 text-accent" /> برند قطعه
              </Link>
            </div>
          </div>
        </section>
        ...
        <section id="parts-directory" className="scroll-mt-24 pt-10 sm:pt-12">
          <div className="mb-7 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-bold text-accent">فهرست محصولات</p>
