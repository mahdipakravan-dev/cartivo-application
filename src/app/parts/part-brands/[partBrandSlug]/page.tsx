...
export default async function PartBrandDetailPage({
  ...
  return (
    <main className="bg-[var(--background)] pb-20 pt-24 sm:pt-28">
      <div className="container-cartivo px-4 sm:px-6 lg:px-8">
        ...
        >
          <ol className="flex items-center gap-1.5 whitespace-nowrap">
            ...
            <li className="truncate font-bold text-text-muted">{name}</li>
          </ol>
        </nav>
        ...
        <section className="relative isolate overflow-hidden rounded-[2rem] bg-primary shadow-[0_24px_70px_rgb(15_23_42/0.12)]">
          ...
          <div className="grid min-h-[400px] lg:grid-cols-[1.08fr_.92fr]">
            <div className="relative z-10 flex items-center px-6 py-12 sm:px-10 lg:px-14">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-bold text-accent">
                  ...
                  تولیدکننده قطعات یدکی
                </div>
                <h1 className="mt-6 text-3xl font-black leading-[1.4] text-white sm:text-4xl lg:text-5xl">
                  قطعات برند<span className="block text-accent">{name}</span>
                </h1>
                {brand.englishName && brand.englishName !== name && (
                  <p
                    dir="ltr"
                    className="mt-2 text-sm font-bold uppercase tracking-[0.18em] text-white/35"
                  >
                    {brand.englishName}
                  </p>
                )}
                <p className="mt-5 max-w-lg text-sm leading-7 text-white/60 sm:text-base">
                  محصولات موجود این تولیدکننده را بررسی کنید و با فیلترهای دقیق،
                  قطعه مناسب را پیدا کنید.
                </p>
                <div className="mt-8 flex flex-wrap gap-3">
                  <a
                    href="#manufacturer-parts"
                    className="inline-flex h-12 items-center gap-2 rounded-xl bg-white px-5 text-sm font-extrabold text-[var(--primary)]"
                  >
                    <Search className="size-4" /> مشاهده محصولات{" "}
                    <ArrowDownLeft className="size-4" />
                  </a>
                  <span className="inline-flex h-12 items-center gap-2 rounded-xl border border-white/15 bg-white/10 px-4 text-xs font-bold text-white/70">
            ...
            <div className="relative hidden items-center justify-center border-r border-white/10 lg:flex">
              <div className="absolute size-[330px] rounded-full border border-white/10" />
              <div className="absolute size-[250px] rounded-full border border-dashed border-accent/15" />
              <div className="relative flex size-52 rotate-[-3deg] items-center justify-center rounded-[2.5rem] border border-white/20 bg-white p-8 shadow-2xl">
                {brand.iconUrl ? (
                  <Image
                    src={brand.iconUrl}
                    alt={`لوگوی ${name}`}
                  ...
                  <Factory className="size-20 text-[var(--primary)]" />
                )}
              </div>
              {brand.countryCode && (
                <span
                  dir="ltr"
                  className="absolute left-10 top-10 rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-xs font-black tracking-widest text-white/70"
                >
                  {brand.countryCode}
                </span>
              )}
      ...
      <section id="manufacturer-parts" className="scroll-mt-24 py-10 sm:py-12">
        <div className="container-cartivo px-4 sm:px-6 lg:px-8">
          ...
          <div className="rounded-[1.75rem]  bg-white p-4 shadow-[0_16px_50px_rgb(15_23_42/0.045)] sm:p-6 lg:p-8">
            <Suspense
              fallback={
