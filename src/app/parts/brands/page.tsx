...
export const metadata: Metadata = {
  ...
  description:
    "لیست کامل برندهای خودرو در کارتیوُ؛ سایپا، ایران‌خودرو، کیا، هیوندای و… . برند خودروی خود را انتخاب کنید و قطعات یدکی سازگار با مدل و سال ساخت را با مقایسه قیمت فروشندگان بخرید.",
      ...
      <main className="bg-[var(--background)] pb-20 pt-24 sm:pt-28">
        <div className="container-cartivo px-4 sm:px-6 lg:px-8">
          ...
          <section className="relative isolate overflow-hidden rounded-[2rem] bg-primary shadow-[0_24px_70px_rgb(15_23_42/0.12)]">
            ...
            <div className="grid min-h-[390px] lg:grid-cols-[1.05fr_.95fr]">
              <div className="relative z-10 flex items-center px-6 py-12 sm:px-10 lg:px-14 lg:py-16">
                <div className="max-w-xl">
                  ...
                  <p className="mt-5 max-w-lg text-sm leading-7 text-white/65 sm:text-base sm:leading-8">
                    برند خودرو را انتخاب کنید تا فقط قطعات سازگار با مدل‌های
                    همان برند را ببینید؛ سریع‌تر، دقیق‌تر و بدون حدس‌زدن.
                  </p>
                  <a
                    href="#brand-directory"
                    className="mt-8 inline-flex h-12 items-center gap-2 rounded-xl bg-white px-5 text-sm font-extrabold text-[var(--primary)] shadow-lg shadow-black/10 transition hover:-translate-y-0.5 hover:bg-accent"
                  >
              ...
              <div className="relative hidden min-h-[390px] items-center justify-center border-r border-white/10 lg:flex">
                ...
                <div
                  className="relative grid w-[360px] rotate-[-4deg] grid-cols-3 gap-3"
                  aria-hidden="true"
                >
                  {brands.slice(0, 9).map((brand, index) => (
                    <div
                      key={brand.id ?? index}
                      className={`flex aspect-square flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/[0.07] p-3 text-center shadow-xl backdrop-blur-sm ${index === 4 ? "scale-110 border-accent/30 bg-accent/15" : ""}`}
                    >
                      <CarFront className="mb-2 size-5 text-accent/80" />
                      <span className="line-clamp-1 text-xs font-bold text-white/80">
                        {brand.persianName}
                      </span>
                      <span
                        dir="ltr"
                        className="mt-0.5 line-clamp-1 text-[9px] uppercase tracking-wider text-white/35"
                      >
                        {brand.englishName}
                      </span>
                    </div>
          ...
          <section id="brand-directory" className="scroll-mt-24 pt-10 sm:pt-12">
            <div className="mb-7">
              <p className="text-xs font-bold text-accent">فهرست برندها</p>
              <div className="mt-2 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
                <h2 className="text-2xl font-black text-dark sm:text-3xl">
                  خودروی شما ساخت کدام برند است؟
                </h2>
                <p className="text-sm text-text-secondary">
                  با جست‌وجو یا کشور سازنده، سریع‌تر انتخاب کنید.
                </p>
              </div>
            </div>
