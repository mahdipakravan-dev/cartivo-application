...
export async function generateMetadata({
  ...
  params: Promise<{ brandSlug: string; partId: string }>;
}): Promise<Metadata> {
  const { brandSlug, partId } = await params;
  const [brand, part] = await Promise.all([
    getBrandBySlug(brandSlug),
    getPartById(Number(partId)),
  ]);

  if (!brand || !part) return { title: "قطعه یافت نشد" };

  return {
    title: `${part.name} — ${brand.persianName} | ${siteConfig.name}`,
    description: `خرید ${part.name} مناسب خودروهای ${brand.persianName} با بهترین قیمت.`,
    alternates: {
      canonical: `/parts/${brandSlug}/${partId}`,
    },
    openGraph: {
      title: `${part.name} — ${brand.persianName} | ${siteConfig.name}`,
      description: `خرید ${part.name} مناسب خودروهای ${brand.persianName}.`,
      url: `/parts/${brandSlug}/${partId}`,
      type: "website",
    },
  };
}

export default async function PartsPartSimplePage({
  params,
}: {
  params: Promise<{ brandSlug: string; partId: string }>;
}) {
  ...
  if (!brand || !part) notFound();

  const partJsonLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    name: part.name,
    description: `${part.name} مناسب خودروهای ${brand.persianName}`,
    brand: {
      "@type": "Brand",
            ...
            <div className="lg:col-span-2">
              <Card>
                <CardContent className="p-6 sm:p-8">
                  ...
                  <div className="mt-8 border-t border-slate-100 pt-8">
                    <Link
                      href={ROUTES.partsBrand(brandSlug)}
                      className="block rounded-xl border border-slate-100 bg-slate-50 p-4 transition-all hover:border-slate-200 hover:shadow-sm"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-800">{brand.persianName}</p>
                          <p className="text-xs text-slate-400">مشاهده همه خودروها و قطعات</p>
                        </div>
                      </div>
                    </Link>
                  </div>
